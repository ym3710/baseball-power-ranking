...
2026年8月23日、1回のセッション内で121校から200校まで一気に拡張した（ユーザーの「一気に200校いこう」という指示による）。北海道・東北・関東・甲信越・東海・近畿・中国・四国・九州・沖縄と全国をまんべんなくカバーするよう意識して選定した。検証作業中、同名だが別の都道府県の学校（例: 大阪の「池田」、愛知の「中村」）を誤って参照しそうになるケースが複数回あったため、取得したページの学校名・都道府県が意図した学校と一致するか毎回確認するようにしている。データが見つからない・出典ページの学校が一致しないと判断した候補校（例: 御所実業、大分工業、佐世保実業、桜丘、岡崎城西、中部大春日丘など）は今回見送っている。

**掲載範囲の方針（2026年8月23日確定）:** 一度「高野連加盟校すべて（甲子園出場歴の有無を問わず）」を検討したが、それだと大多数が優勝0・出場0・勝利0で並んでしまいスコアに意味がなくなること、全国約4000校の一括検証は非現実的であることから見送った。最終的な方針は「甲子園（春または夏の全国大会）に一度でも出場したことがある学校は、いずれ全て掲載する」。出場歴が一度もない学校は完全に対象外。1回のセッションで全て終わらせるものではなく、同じ検証方法（baseball.yahoo.co.jpの各校ページで春夏の出場・優勝・勝利数を確認）でセッションをまたいで継続的に増やしていく。
